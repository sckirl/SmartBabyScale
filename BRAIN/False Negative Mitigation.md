...
> [!WARNING] Sample Weight Inversion Flag
> The recent [[IEEE Reviewer Audit]] flagged that the `sample_weight = Y + 1.0` formula is potentially **inverted** relative to clinical need! By utilizing this specific mapping, we might inadvertently upweight heavy (term) healthy infants rather than high-risk VLBW (Very Low Birth Weight) infants, which contradicts the core clinical asymmetry. An immediate ablation study is required to verify the directional impact of the weights.
